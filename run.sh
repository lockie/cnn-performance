...
cd cuda-convnet
echo "*** Training net"
python convnet.py --data-path=$cwd/CBCL_DATA --save-path=/tmp --test-range=6 --train-range=1-5 --layer-def=$cwd/layers.cfg --layer-params=$cwd/params.cfg --data-provider=cbcl --test-freq=13 --epochs=60 | tee /tmp/train.log
echo "*** Testing net"
